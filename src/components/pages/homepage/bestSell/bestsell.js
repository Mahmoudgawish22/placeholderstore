...
function BestSell(props) {
  ...
  const userWeNeed = useSelector((state)=> state.users).filter(item=> item._id==user?.result._id);
    ...
    const bestEight = soreted.slice(soreted.length-8);
    ...
    return (
        <div id='best_sell'>
           <div id='offers__title' style={{padding: '20px', paddingBottom: '5px'}}>
                <label style={{fontSize: '27px'}} className='section__title'><i className="fas fa-fire"></i> Best Seller</label>
                <a className='seemore' style={{cursor: 'pointer'}} onClick={()=> openPosts('forbest')}>See More</a>
            </div>
            {display2? 
            ...
            <Carousel.Item>
               ...
               <div className='products__slideShow' style={{backgroundColor: '#eaeaea'}}>
                 {bestEight.slice(bestEight.length-4).map(item=> 
