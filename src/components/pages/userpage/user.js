...
export default function User(props) {
  const [user, setUser] = useState(JSON.parse(localStorage.getItem('profile')));
    ...
    const [signFormData, setSignFormData] = useState({
      ...
      birthday: user.result.dateOfBirth,
      country: user.result.country,
      phone: user.result.phone,
      photo: user.result.photo,
      userStatus: user.result.userStatus,
      dateOfRigester: user.result.dateOfRigester,
      liked: user.result.liked,
      cart: user.result.cart,
      recentlyView: user.result.recentlyView,
      orders: user.result.orders,
    })
    ...
    const handleSignUpSubmit = (e) => {
      ...
      dispatch(updateUser(user?.result._id, signFormData, setDoneModal, setLoadingModalShow, setErrorLoginRespond));
    }
    if (user?.result?.name) {
    return (
        <div id='user'>
            <img src={user?.result.photo} className='user_profile' alt='user_profile'/>
            {display==0?
            <div id='user_info'>
            <label className='form_label'>{user?.result.name}</label>
            <label className='form_label'>{user?.result.email}</label>
            <label className='form_label'>{user?.result.address}</label>
            <label className='form_label'>{user?.result.dateOfBirth}</label>
            <label className='form_label'>{user?.result.phone}</label>
            <label className='form_label'>{user?.result.country}</label>
            <button style={{alignSelf: 'flex-end'}} onClick={()=> setDisplay(1)} className='btn btn-block btn-dark'>Edit</button>
            </div> :
        ...
        <DoneModal
          show={doneModal}
          textStatus= 'Done!'
          onHide={() => {setDoneModal(false); window.location.reload()}}
        />
        <LoadingModal
